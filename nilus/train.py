...
iterations = 10
image_scale = 0.01
image_len = image_x*image_y*image_scale
...
def combine_layer(inp0, inp1 ): #,inp2, inp3, inp4, inp5, inp6, inp7
    ...
    comb_layer = tf.concat([inp0,inp1], 0) #, inp2,inp3, inp4, inp5, inp6, inp7
    return comb_layer
...
def comb_conv(input):
    ...
    return logits7
    '''
    tmp = np.prod(dropout1.shape.as_list()[1:])
    flattened = tf.reshape(dropout1, [-1, tmp])
    '''
    # Dense Layer # 1
    dense_layer = tf.layers.dense(inputs=flattened, units=65536, activation=tf.nn.relu)
    logits = tf.layers.dense(inputs=dense_layer, units=262144)
    '''
    dense_out = tf.layers.dense(flattened,
                                int(64*64*64/100),
                                activation=tf.nn.sigmoid)
    logits = tf.layers.dense(inputs=dense_out, units=64*64*64)
    return logits
...
def get_batch(data, size):
    ...
    for i in range(size):
        ...
        for l in range(len(npz.files)):
            ...
            image = scipy.misc.imresize(image, image_scale, interp='bilinear', mode=None)
            depth_maps.append(image.flatten())
...
if __name__ == "__main__":
    ...
    print("got data")
    print(image_len)
    x_0 = tf.placeholder(tf.float32, [None, image_len])
    print("x_0 done")
    '''
    x_1 = tf.placeholder(tf.float32, [None, image_len])
    print("x_1 done")
    ...
    tmp = np.prod(conv_0.shape.as_list()[1:])
    flattened = tf.reshape(conv_0, [-1, tmp])
    '''
    # Dense Layer # 1
    dense_layer = tf.layers.dense(inputs=flattened, units=65536, activation=tf.nn.relu)
    logits = tf.layers.dense(inputs=dense_layer, units=262144)
